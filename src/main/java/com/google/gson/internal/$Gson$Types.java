...
public final class $Gson$Types {
    ...
    static Type getGenericSupertype(Type type, Class<?> object, Class<?> class_) {
        ...
        if (class_.isInterface()) {
            ...
            for (int i2 = 0; i2 < n2; ++i2) {
                ...
                return $Gson$Types.getGenericSupertype(object.getGenericInterfaces()[i2], class_2[i2], class_);
            }
        }
    ...
    public static Type resolve(Type type, Class<?> class_, Type type2) {
        ...
        if (type2 instanceof ParameterizedType) {
            ...
            for (int i2 = 0; i2 < n2; ++i2) {
                Type type8 = $Gson$Types.resolve(type, class_, arrtype[i2]);
    ...
    ParameterizedType {
        ...
        public /* varargs */ ParameterizedTypeImpl(Type type, Type type2, Type ... arrtype) {
            ...
            for (int i2 = 0; i2 < this.typeArguments.length; ++i2) {
                $Gson$Preconditions.checkNotNull(this.typeArguments[i2]);
                $Gson$Types.checkNotPrimitive(this.typeArguments[i2]);
                this.typeArguments[i2] = $Gson$Types.canonicalize(this.typeArguments[i2]);
            }
        }
